...
3. ~~**Conversion**: the landing "coming soon" waitlist is a `mailto:`~~ — waitlist half SHIPPED `6e4f09b` (2026-06-10): one-field EN/AR form → `/api/waitlist` (per-IP rate limit) → Convex `waitlist` table, surfaced in admin Overview with copy-all. Still open: pricing section surfacing the creator/developer demos ("see it play").
4. ~~**Builder**: drag-to-reorder for experience/projects items~~ — reorder half SHIPPED `46b7b6d`+`3b5980e` (2026-06-10): every DynamicList (10 builder steps) has a grip handle — framer-motion Reorder drag (mouse + touch, `touch-action:none`) plus ArrowUp/Down keyboard reorder on the focused handle, EN+AR labels. E2E-verified desktop EN/AR + 390px touch. Still open: per-template AI Fill on the first step of engineer/creative/creator/developer (the shared CV-step AI exists, but the full-CV generator is only surfaced on general's Basics step).
5. ~~**AI usage limits** are client-state only~~ — SHIPPED `3bbc28d` (2026-06-10): durable per-user daily caps via the Convex rate-limiter (15 summaries / 9 full-CVs per 24h — 3x the client session allowance) on top of the existing per-minute burst limits; 429s carry a "try again tomorrow" message and the builder shows a localized EN/AR error.
