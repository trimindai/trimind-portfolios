...
- ✅ **3.7 — Password show/hide toggle** on sign-in + sign-up — `858882b`. `showPassword` state, `type` toggles, `type="button"` eye (won't submit), RTL-safe logical `end-3`/`pe-11`, bilingual aria-labels, `aria-pressed`. tsc exit 0. Spec + quality APPROVED. 2 files only.
- ✅ **3.3 — Reviewer initials-avatars on endorsements** — `7290bd0`. `general/template.hbs` endorsement-author → flex row with `.endorsement-avatar` gradient circle (`{{initials this.name}}`), gap-based (RTL-safe), print/mobile handled. Compile+render verified. 1 file only. (Testimonials.tsx is a trust-signal feature strip with no reviewers → correctly out of scope.)
- ✅ **3.2 — Cert badges clickable→expand: RESOLVED N/A (data-limited).** Cert model is `{name, issuer, year?}`; the general credentials bar ALREADY shows `cred-acronym` (name) + `cred-issuer`. Only optional `year` is unshown — no rich hidden detail (no descriptions/IDs/links) to justify a clickable-expand. Not warranted. (Backlog micro-tweak: show `year` inline if ever desired.)
