...
1. **Part 3 polish — CLEAN real-product batch** (themed commits; DEFER all demo-EN items to batch):
   - 3.4 footer "Built with…" link `target="_blank" rel="noopener"` across engineer/creative/developer/creator `template.hbs` (general done).
   - 3.5 phone `tel:` links across templates that show a phone (general/_cv done).
   - 3.10 footer copyright em-dash "—" consistency across `template.hbs` footers.
   - 3.15 brand "Portfolio Pro" → «بورتفوليو برو» when AR — find landing header/nav component(s) + any template/nav brand string; localize on `isAr`.
   - 3.1 stronger nav hover (underline/color shift) in `template.hbs` nav styles.
   - (Second pass / more involved, real-product, can be a separate commit) 3.3 reviewer initials-avatars on testimonials (`general/template.hbs` endorsements + `src/components/landing/Testimonials.tsx`, reuse initials), 3.2 cert badges clickable→expand (`general/template.hbs` credentials), 3.7 password show/hide toggle (sign-in + sign-up pages).
   - Verify-only: 3.13 (`/ar` page titles via generateMetadata — confirm Arabic), 3.14 (Arabic email placeholder — likely no-op). Skip: 3.8 (Remember me). Done: 3.6/3.9/3.12.
2. **Part 5 verify** — `scripts/portfolio-fix-verify.mjs` (NEW/clean): codify the QA acceptance checks (auth, /ar demos 200+rtl, /ar templates, guest /try, real CV+QR, engineer projects, polish). Run against preview/local; report.
3. **EN demo static-file batch** (deferred): PDF/QR button + sample-cv assets (author correct personas) + initials avatar + Part-3 demo chrome (nav hover/footers/tel/brand/LinkedIn 3.11) + engineer EN nav repoint, applied to each EN `public/demo/<id>/index.html` once unblocked.
