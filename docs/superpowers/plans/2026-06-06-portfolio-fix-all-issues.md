...
1. **Task 2.2** (Download CV PDF + scannable QR on demos). Clean parts: `scripts/build-demo-cv-assets.mjs` (reuse `renderCvPdf` from `src/app/api/generate-cv/route.ts` + `portfolioQrDataUrl` from `src/lib/qr.ts`) → emit `public/demo/<id>/qr.png` + `sample-cv.*` (NEW files, clean); wire the REAL builder/preview "Save/Download PDF" to `/api/generate-cv` (already wired there per plan — verify); AR demos (clean) can get the button+QR block. **DEFER** the EN demo `index.html` Download-CV button + QR block to the EN-demo batch.
2. **Part 3 polish** (clean-file items first: template `.hbs` nav hover/footers/tel/brand-localization/cert-badges/reviewer-avatars; demo-EN items → batch) → **Part 5 verify** (`scripts/portfolio-fix-verify.mjs`, NEW/clean).
3. **EN demo static-file batch** (deferred): PDF/QR button + initials avatar + Part-3 demo chrome + engineer EN nav repoint, applied to each EN `public/demo/<id>/index.html` once the other session commits its og edits (or coordinate with user).
