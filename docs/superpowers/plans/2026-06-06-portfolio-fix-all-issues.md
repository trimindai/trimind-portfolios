...
**⚠️ FOLLOW-UPS discovered this session:**
...
- ✅ **Task 2.8 — Homepage form respects template choice** — `c63f47f`. `TryItForm` no longer forces `?template=corporate`; routes to `/[locale]/templates?prefill=1`. Templates page (server comp): Next-15 async `searchParams`, bilingual prefill banner (joined into `t`), per-card "Use" CTA routes by auth — **signed-in → `/dashboard/new?template=<id>` (unchanged), signed-out → `/try/<id>` (public guest builder, no more sign-in wall)**. `/try` page seeds `basics` from `portfolio-draft` (SSR-guarded, non-destructive; BuilderForm still prefers `portfolio_preview_data`). tsc exit 0. Spec ✅ + code-quality APPROVED. 3 files only.

**🚧 SHARED-TREE STRATEGY for remaining Part 2/3 (READ — avoids wasted blocked work):**
The EN demo `public/demo/<id>/index.html` files (corporate, creative, creator, developer, engineer) are **dirty** with ANOTHER session's uncommitted og:image/twitter meta edits. `git add -p` (interactive hunk staging) is unavailable in this harness, so I **cannot cleanly stage my own hunk** in those files without also committing the other session's work (forbidden). Therefore, for 2.2/2.5/2.6/Part-3:
- **DO the clean parts now:** real-product templates `src/templates/<id>/template.hbs` (CLEAN), new files (`scripts/build-demo-cv-assets.mjs`, generated `public/demo/<id>/qr.png`+`sample-cv.*` are NEW/clean), the **AR** demo files `index-ar.html` (mine/committed → editable+stageable), builder/preview components, and `scripts/portfolio-fix-verify.mjs` (new).
...
- ✅ **Task 2.5 — Gradient initials avatar fallback** — `cfdfb7e` + dead-CSS cleanup `b28cc37`. Reused existing `initials` helper (template-engine.ts:154, unchanged). Replaced/added the no-photo `{{else}}` branch with `<div class="avatar-initials" aria-label=…>{{initials basics.fullName}}</div>` + accent-var gradient CSS in general/engineer/creative/creator `template.hbs` (developer hero has no photo → skipped); general caption "Professional Photo" → fullName. Builder `PhotoUpload.tsx` empty-state → initials (Arabic-safe `computeInitials`) + `darken()` gradient + hover camera hint; wired `name`/`accentColor` from CustomizeStep + EngineerCustomizeStep. tsc exit 0; standalone Handlebars render check passed for all 4 (initials present, silhouette gone, with-photo `<img>` intact). Spec ✅ + code-quality APPROVED. AR demos: no silhouettes to replace (skipped). EN demos → deferred to batch.
  - Follow-up nits (not blocking): PhotoUpload upload trigger is a `<div onClick>` (mouse-only, pre-existing a11y gap); white-on-light-accent contrast edge case if a user picks a very pale accent.

- ✅ **Task 2.2a — Real builder/preview CV PDF + QR: VERIFIED WORKING (no change).** Code-path confirmed: preview page (`dashboard/[id]/preview/page.tsx`) "Download PDF"/«حمّل PDF» button (`canDownload`, payment/publish-gated) → `printCv()` switches to CV view → `PreviewFrame.print()` opens the CV HTML in a new window+blob URL and calls `contentWindow.print()` (mobile-safe). CV HTML from `/api/generate-cv` embeds the scannable QR server-side via `renderCvPdf(data,{qrDataUrl,liveUrl})` → live portfolio URL. No published-template "Save PDF" button exists (download is the gated preview button — correct). The real-user differentiator works.
- ⏸️ **Task 2.2b — Demo CV assets + Download/QR buttons: DEFERRED to EN-demo batch.** Reasons: (1) EN demo `index.html` buttons blocked (dirty w/ other session); (2) `DEMO_DATA` in `src/app/demo/[templateId]/route.ts` is BOTH dirty AND **stale** (engineer there = old Omar-Sabah/mechanical, not the live Abdulrahman/electrical; likely dev/creative too) — so a `build-demo-cv-assets` script must author CORRECT per-persona CV data matching the live static demos, not import the stale DEMO_DATA. Best done coherently with the EN-demo batch. (`tsx` IS available for such a script when picked up.)

- ✅ **Part 3 polish — template.hbs cosmetic batch** — `74e3d48` (5 files, +14/−7; compile-verified). 3.1 nav hover strengthened (accent color + underline / animated underline) across all 5 templates; 3.10 footer em-dash "—" normalized (creative/developer/creator); 3.4 developer footer gained a `target="_blank" rel="noopener"` credit link (general/creator already had it; engineer/creative footers have no external credit → skipped); 3.5 all templates that render a phone already had `tel:` links (no change needed). Only the 5 `template.hbs` committed, no foreign files.
  - 3.15 (brand → «بورتفوليو برو») DEFERRED: the "Portfolio Pro/TriMind" brand string is in `src/app/[locale]/page.tsx` (DIRTY, other session) — can't cleanly stage. Do once page.tsx is committed.

**RESUME HERE (in order):**
1. **Part 3 — remaining CLEAN real-product items** (separate themed commits; verify each file `git status` clean first):
   - 3.7 password show/hide toggle on sign-in + sign-up pages (`src/app/[locale]/(app)/sign-in/[[...sign-in]]/page.tsx`, `…/sign-up/[[...sign-up]]/page.tsx`) — add an eye button toggling input `type`. Real React → full two-stage. (Confirm pages clean; they were committed in 1.2/2.4.)
   - 3.3 reviewer initials-avatars on testimonials — `src/components/landing/Testimonials.tsx` (CLEAN) + `general/template.hbs` endorsements section; reuse the initials/avatar treatment from 2.5.
   - 3.2 cert badges clickable→expand details — `general/template.hbs` credentials section (+ a tiny `<details>`/JS toggle).
   - Verify-only: 3.13 (`/ar` page titles via `generateMetadata` — confirm Arabic present), 3.14 (Arabic email placeholder — likely no-op). Skip: 3.8 (Remember me). Done: 3.1/3.4/3.5/3.6/3.9/3.10/3.12. Deferred: 3.15 (page.tsx dirty), 3.11 LinkedIn-on-engineer-demo (EN demo batch).
2. **Part 5 verify** — `scripts/portfolio-fix-verify.mjs` (NEW/clean): codify QA acceptance checks (auth; /ar demos 200+rtl+arabic; /ar templates; guest /try; real CV+QR; engineer projects). Run one-shot against local/preview; report.
3. **EN demo static-file batch** (deferred): 2.2b (PDF/QR button + sample-cv assets w/ correct personas) + initials avatar + Part-3 demo chrome (nav hover/footers/tel/brand/LinkedIn 3.11) + engineer EN nav repoint, applied to each EN `public/demo/<id>/index.html` once unblocked (other session commits its og edits, or user coordinates).
